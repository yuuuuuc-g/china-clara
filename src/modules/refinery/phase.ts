...
function buildSystemPrompt(phase: RefineryPhase, topicTitle?: string): string {
  return phase === "D" && topicTitle ? buildPhaseDPrompt(topicTitle) : PHASE_PROMPTS[phase];
}
...
export function runRefineryPhase(input: RunRefineryPhaseInput) {
  return streamText({
    ...
    model: input.model,
    system: buildSystemPrompt(input.phase, input.topicTitle),
    prompt: input.prompt,
    temperature: 0.35,
    tools: {
